<template>
  <div class="storage-file-explorer flex flex-col h-[520px] text-xs text-gray-800 dark:text-gray-200">
    ...
    <div class="flex items-center space-x-2 pb-3 border-b border-gray-200 dark:border-gray-700">
      ...
      <button
        ...
        @click="loadFiles"
      >
        <v-remixicon name="riRefreshLine" size="14" :class="{ 'animate-spin': isLoading }" />
    ...
    <div class="flex-1 overflow-y-auto py-3 space-y-4">
      <div v-if="isLoading" class="py-12 text-center text-gray-400">
        <v-remixicon name="riLoader4Line" size="20" class="animate-spin inline-block mb-1" />
        <p>Scanning Storage workspace...</p>
      </div>

      <div v-else-if="filteredFiles.length === 0" class="py-12 text-center text-gray-400">
        <v-remixicon name="riFolderOpenLine" size="24" class="inline-block mb-2 text-gray-300 dark:text-gray-600" />
        <p class="font-medium">No matching files found in Storage</p>
        <p class="text-[11px] text-gray-400 mt-1">Make sure automa-core is running and files exist in workspace</p>
      </div>

      <div v-else class="space-y-3">
        <!-- Workflows Section -->
        <div v-if="workflows.length > 0">
          <div class="flex items-center space-x-1.5 text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5 px-1">
          ...
          <div class="grid grid-cols-1 gap-1.5">
            ...
            >
              <div class="flex items-center space-x-2.5 min-w-0 pr-2">
                <div class="w-7 h-7 rounded-md bg-blue-50 dark:bg-blue-950/60 text-accent flex items-center justify-center shrink-0">
                  <v-remixicon name="riFlowChart" size="16" />
                </div>
                <div class="flex flex-col min-w-0">
                  <span class="font-medium text-xs text-gray-900 dark:text-gray-100 truncate group-hover:text-accent transition">
                    {{ file.name }}
                  </span>
                  <span class="text-[11px] text-gray-400 dark:text-gray-500 font-mono truncate">
                    {{ file.relative_path }}
                  </span>
                </div>
              </div>
        ...
        <div v-if="campaigns.length > 0">
          <div class="flex items-center space-x-1.5 text-[11px] font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5 px-1 pt-2">
            <v-remixicon name="riRocketLine" size="13" class="text-emerald-500" />
          ...
          <div class="grid grid-cols-1 gap-1.5">
            ...
            >
              <div class="flex items-center space-x-2.5 min-w-0 pr-2">
                <div class="w-7 h-7 rounded-md bg-emerald-50 dark:bg-emerald-950/60 text-emerald-500 flex items-center justify-center shrink-0">
              ...
              <span class="text-[10px] font-semibold px-2 py-0.5 rounded bg-emerald-100 dark:bg-emerald-900/60 text-emerald-700 dark:text-emerald-300">
                Campaign
              </span>
            </div>
          </div>
    ...
    <!-- Footer Bar -->
    <div class="pt-3 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between text-gray-500">
